...
class GenRouter:
    ...
    def get_router(self) -> APIRouter:
        ...
        router.add_api_route("/start", self.get_started, methods=["GET"])
        router.add_api_route("/upload-document", self.upload_document, methods=["GET"])
        router.add_api_route("/configure-presntation", self.config_presentation, methods=["GET"])
        router.add_api_route("/view-slides", self.view_slides, methods=["GET"])
        router.add_api_route("/preview", self.preview_slides, methods=["GET"])

        return router

    def get_started(self, request: Request) -> HTMLResponse:
        context = utils.PageContext(
            next_step=request.url_for("upload_document"),
            request=request,
            active_page=1,
        ).dict()
        return self.templates.TemplateResponse("home.html", context)
