...
class User(BaseModel, table=True):
    ...
    telegram_username: Optional[str] = Field(String(32), nullable=True, 
                                                    comment="Telegram username (if set)")
    first_name: str = Field(String(64), nullable=False, 
                                   comment="User's first name from Telegram")
    last_name: Optional[str] = Field(String(64), nullable=True, 
                                            comment="User's last name from Telegram")
    language_code: Optional[str] = Field(String(10), nullable=True, 
                                                comment="User's language code (e.g., 'en')")

    is_active: bool = Field(Boolean, default=True, 
                                   comment="Whether the user is currently active")

    last_interaction: Optional[datetime] = Field(DateTime, nullable=True, 
                                                        comment="Last time user interacted with bot")
    chat_id: int = Field(Integer, nullable=False, 
                                comment="Telegram chat ID for messaging")

    email: Optional[EmailStr] = Field(String(255), nullable=True, 
                                             comment="User's email address")
    phone_number: Optional[str] = Field(String(20), nullable=True, 
                                               comment="User's phone number")
    timezone: Optional[str] = Field(String(50), nullable=True, 
                                           comment="User's timezone (e.g., 'UTC', 'America/New_York')")

    # Bot usage tracking
    command_count: int = Field(Integer, default=0, 
                                      comment="Number of commands executed by user")
    is_blocked: bool = Field(Boolean, default=False, 
